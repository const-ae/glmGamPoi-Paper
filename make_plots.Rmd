...
# p_dur_rel_pbmc4k <- pbmc4k_bench_res %>%
#   filter(method != "DESeq2 glmGamPoi") %>%
#   mutate(method = factor(method, levels = c("glm_gp", "glm_gp on disk",
#                                              "glm_gp overdispersion = 0",
#                                              "DESeq2 with glmGamPoi",
...
#     stat_summary(geom = "col", fun.y = median, fill = "lightgrey") +
#     ggbeeswarm::geom_quasirandom(groupOnX = TRUE) +
#     annotate("text", x = 6.3, y = 17, label = "33,694 genes x 4,340 cells", hjust = 1, vjust = 1) +
#     coord_flip() +
...
p_dur_rel_mouse <- mouse_bench_res %>%
  mutate(method = factor(method, levels = c("glm_gp", "glm_gp on_disk",
                                             "glm_gp overdispersion = 0",
                                             "DESeq2 with glmGamPoi",
                                             "edgeR", "DESeq2"), ordered = TRUE)) %>% 
  ...
  ggplot(aes(y = relative_time, x = method)) +
    ...
    ggbeeswarm::geom_quasirandom(groupOnX = TRUE) +
    geom_hline(yintercept = 1) +
    annotate("text", x = 6.3, y = 17, label = "29,453 genes x 30,703 cells", hjust = 1, vjust = 1) +
    coord_flip() +
    scale_y_continuous(expand = expand_scale(add = c(0, 0)),
                       limits = c(0, 17),
                       breaks = c(0.5, 1, 2, 4, 8, 16),
...
p_dur_rel_brain <- brain_bench_res %>%
  mutate(method = factor(method, levels = c("glm_gp", "glm_gp on disk",
                                             "glm_gp overdispersion = 0",
                                             "DESeq2 with glmGamPoi",
                                             "edgeR", "DESeq2"), ordered = TRUE)) %>% 
  ...
  ggplot(aes(y = relative_timing, x = method)) +
    ...
    ggbeeswarm::geom_quasirandom(groupOnX = TRUE) +
    geom_hline(yintercept = 1) +
    annotate("text", x = 6.3, y = 16, label = "27,998 genes x 20,000 cells", hjust = 1, vjust = 1) +
    coord_flip() +
    scale_y_continuous(expand = expand_scale(add = c(0, 0)),
                   limits = c(0, 16),
                   breaks = c(0.5, 1, 2, 4, 8, 16),
...
cowplot::plot_grid(
  cowplot::plot_grid(p_dur_pbmc4k, p_dur_mouse, p_dur_brain, nrow = 1, align = "v",
                    labels = c("PBMC", "Gastrulation", "Brain")),
  cowplot::plot_grid(p_dur_rel_pbmc4k, p_dur_rel_mouse, p_dur_rel_brain, nrow = 1, align = "v",
                     labels = c("PBMC", "Gastrulation", "Brain")),
  nrow = 2
)

cowplot::save_plot("plots/three_duration_plots.pdf", last_plot(), ncol = 3, nrow = 2)
...
p_pbmc4k_coef <- pbmc4k_coef_df %>%
  ...
  group_map(function(df, method){
    p <-  ggplot(df, aes(x = glm_gp, y = value)) +
      # ggpointdensity::geom_pointdensity(method = "default", size = 0.5) +
      geom_abline(intercept = 0, slope = 1) +
      ggpointdensity::stat_pointdensity(geom = ggrastr:::GeomPointRast, method = "default", size = 1.5) +
      scale_color_viridis_c(guide = "none") +
  ...
  cowplot::plot_grid(plotlist = ., nrow = 1, rel_widths = c(1.2, 1, 1))
...
```{r}
tmp <- assay(sce)[str_replace(very_significant_genes_df$gene, "gene_", "") %>% as.numeric(), ] 
rownames(tmp) <- very_significant_genes_df$gene
very_significant_genes_df$max_count <- rowMaxs(tmp)

p_comp_similar <- as_tibble(t(tmp)) %>%
  mutate(mouse = colData(sce)$Mouse,
         cell = seq_len(n())) %>%
  pivot_longer(cols = starts_with("gene_"), names_to ="gene", values_to = "count") %>%
...
tmp2 <- assay(sce)[str_replace(very_different_genes_df$gene, "gene_", "") %>% as.numeric(), ] 
rownames(tmp2) <- very_different_genes_df$gene
