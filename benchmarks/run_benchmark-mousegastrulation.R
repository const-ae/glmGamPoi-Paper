...
# sce <- sce[sample(seq_len(nrow(sce)), size = 1000), sample(seq_len(ncol(sce)), size = 2000)]
...
system.time({
  fit_disp0 <- glm_gp(sce, design =  ~ pool + stage + tomato,
                overdispersion = 0, overdispersion_shrinkage = TRUE,
                size_factors = size_factors, verbose = TRUE)
  res_disp0 <- test_de(fit_disp0, reduced_design = ~ 1)
})
...
print("Fit glm_gp on_disk")

system.time({
  fit_ondisk <- glm_gp(sce, design =  ~ pool + stage + tomato,
                size_factors = size_factors, verbose = TRUE,
                on_disk = TRUE)
  res_ondisk <- test_de(fit_ondisk, reduced_design = ~ 1)
})
...
saveRDS(list(fit_disp0 = fit_disp0, res_disp0 = res_disp0, 
             fit = fit, res = res, 
             fit_ondisk = fit_ondisk, res_ondisk = res_ondisk,
