...
p_de_gp <- tibble(disp_gp = disp_glmgampoi, disp_de = exp(disp_deseq),
  ...
  ggplot(aes(x = disp_gp, y = disp_de)) +
    ...
    scale_x_log10(name = "glmGamPoi", limits = c(8e-10, 1e5), expand = expansion(add = c(0, 0)), 
                  oob = scales::squish,
                  breaks = c(1e-9, 1e-4, 1e0, 1e4),
                  labels = expression(10^-9, 10^-4, 1, 10^4)) + 
    scale_y_log10(name = "DESeq2", limits = c(8e-10, 1e5), expand = expansion(add = c(0, 0)), 
                  oob = scales::squish,
                  breaks = c(1e-9, 1e-4, 1e0, 1e4),
                  labels = expression(10^-9, 10^-4, 1, 10^4)) +
...
p_er_gp <- tibble(disp_gp = disp_glmgampoi, disp_de = disp_edger,
  ...
  ggplot(aes(x = disp_gp, y = disp_de)) +
    ...
    scale_x_log10(name = "glmGamPoi", limits = c(8e-10, 1e5), expand = expansion(add = c(0, 0)), 
                  ...
                  breaks = c(1e-9, 1e-4, 1e0, 1e4),
                  labels = expression(10^-9, 10^-4, 1, 10^4)) + 
    scale_y_log10(name = "edgeR", limits = c(8e-10, 1e5), expand = expansion(add = c(0, 0)), 
                  oob = scales::squish,
                  breaks = c(1e-9, 1e-4, 1e0, 1e4),
                  labels = expression(10^-9, 10^-4, 1, 10^4)) +
    scale_color_discrete() +
    # labs(title = "Dispersion MLE: edgeR vs glmGamPoi") +
    cowplot::theme_cowplot()
```
...
cowplot::plot_grid(
  ...
  cowplot::plot_grid(NULL,
    ...
    NULL, nrow = 1, rel_widths = c(0.8, 1, 1)),
  nrow = 3, rel_heights = c(0.1, 1, 0.1),
  align = "vh"
)
  
cowplot::save_plot("plots/dispersion_mle_comparison.pdf", last_plot(), nrow = 1, ncol = 2, base_asp = 0.9)
```
